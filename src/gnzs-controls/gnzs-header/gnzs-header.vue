<script setup lang="ts">
  ...
  const props = defineProps({
    ...
    // когда внутри амо рендерим область на весь экран (напр, использование левой кнопки меню)
    isInsidePageHolder: {
      type: Boolean,
      default: false,
    },
  ...
  const onShowSidebar = (): void => {
    if (props.isInsidePageHolder) iframeStore.asaidToggle();
  ...
  const focusOut = (event: any): void => {
    const isTargetChild =
      event &&
      event.relatedTarget &&
      editRef.value === event.relatedTarget.closest(`.${style.edit}`);
  ...
  const onSaveCurrentTitle = (): void => {
    emit("onSaveCurrentTitle", { name: input.value, setIsLoad, editClose });
  };

  // computed
  const windowWidth = computed((): number | null => iframeStore.windowWidth);
  const isNotMainPage = computed((): boolean => props.mainRoute !== "");
  ...
  <div :class="$style.root">
    <div :class="[$style.header, { [$style.fixed]: props.fixed && isScrolled }]">
      ...
      <div :class="$style.breadcrumbs">
        <div
          :class="$style.burger"
          v-if="windowWidth && windowWidth <= 1100"
          @click="onShowSidebar"
        >
          <span></span>
        </div>
        <div
          :class="[$style.item, $style.main, { [$style.itsLink]: props.mainRoute.length !== 0 }]"
          @click="goToMainRoute"
        >
          {{ mainTitle }}
        </div>
